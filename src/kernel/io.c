...
int vsprintf(putc_fn_t pc, puts_fn_t ps, const char *fmt, va_list args) {
    ...
    uint8_t len   = PRINTF_LENGTH_DEFAULT;
    uint8_t width = 0;
        ...
        switch (state) {
            ...
            case PRINTF_STATE_SPECIFIER: {
                switch (*fmt) {
                    ...
                    case 'p': len = PRINTF_LENGTH_LONG_LONG;
                    case 'X':
                    case 'x': {
                        uint64_t n = 0;
                        switch (len) {
...
int kprintf(int level, const char *fmt, ...) 
{
    ...
    int ret = vsprintf(serial_putc, serial_puts, level_to_str[level], NULL);
    ret = vsprintf(serial_putc, serial_puts, fmt, args_serial);
    if (IS_INITIALIZED(TTY_MODULE)) 
    {
        ret = vsprintf(tty_putc, tty_puts, level_to_str[level], NULL);
