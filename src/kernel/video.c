...
void init_video(struct framebuffer *fb) {
    fb->base_addr = (void *)P2V((paddr_t)fb->base_addr);
    framebuffer = fb;

    uint32_t clear_color = 0;
    for (size_t i = 0; i < framebuffer->len_scanline * framebuffer->height; ++i) {
        // ((uint32_t*)framebuffer->base_addr)[i] = clear_color;
        vga_set_pixel_index(i, CLEAR_COLOR);
    }
}

struct framebuffer *vga_get_current_framebuffer() {
    return framebuffer;
}
